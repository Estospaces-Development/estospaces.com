...
export function validateBlogPosts(
  posts,
  { expectedCount = 100, sitemapUrls = [], imageRoot = '' } = {},
) {
  const errors = [];
  const seen = {
    slugs: new Set(),
  ...
  posts.forEach((post, index) => {
    ...
    if (!post?.readingTime || post.readingTime < 3)
      errors.push(`${label}: readingTime is too low.`);
    ...
    if (!post?.heroImage?.width || !post?.heroImage?.height)
      errors.push(`${label}: missing hero image dimensions.`);
    ...
    if (!Array.isArray(post?.faq) || post.faq.length < 3)
      errors.push(`${label}: needs at least 3 FAQ entries.`);
    ...
    if (Array.isArray(post?.internalLinks)) {
      post.internalLinks.forEach((link, linkIndex) => {
        if (!link?.title || !link?.href)
          errors.push(`${label}: invalid internal link at index ${linkIndex}.`);
      });
    }
    if (Array.isArray(post?.externalLinks)) {
      post.externalLinks.forEach((link, linkIndex) => {
        if (!link?.title || !link?.url || !link?.publisher)
          errors.push(`${label}: invalid external link at index ${linkIndex}.`);
      });
    }
    if (post?.schemaJsonLd?.['@type'] !== 'BlogPosting') {
    ...
    checkUnique(
      seen.metaDescriptions,
      post?.metaDescription,
      `${label}: duplicate metaDescription.`,
      errors,
    );
    checkUnique(
      seen.targetKeywords,
      post?.targetKeyword,
      `${label}: duplicate targetKeyword.`,
      errors,
    );
    checkUnique(
      seen.searchIntents,
      post?.searchIntent,
      `${label}: duplicate searchIntent.`,
      errors,
    );
    checkUnique(
      seen.heroImageUrls,
      post?.heroImage?.url,
      `${label}: duplicate hero image URL.`,
      errors,
    );
