...
check('100 published posts exist', posts.length === 100 && posts.every((post) => post.status === 'published'));
check('/blogs index is in sitemap', sitemapUrls.includes(`${SITE_URL}/blogs`));
check('robots allows blog crawling', robotsConfig.rules?.allow === '/' && robotsConfig.sitemap === `${SITE_URL}/sitemap.xml`);
check('sitemap includes all 100 blog detail URLs', entries.filter((entry) => entry.url?.startsWith(`${SITE_URL}/blogs/`)).length === 100);
check('sitemap includes all 100 v8 blog images', entries.filter((entry) => entry.images?.some((image) => image.includes('hero-photo-v8.webp'))).length === 100);
...
for (const post of posts) {
  ...
  requireCheck(label, 'slug is clean lowercase URL text', /^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(post.slug));
  requireCheck(label, 'meta title is concise', post.metaTitle.length >= 20 && post.metaTitle.length <= 60);
  requireCheck(label, 'meta description is useful length', post.metaDescription.length >= 80 && post.metaDescription.length <= 156);
  ...
  requireCheck(label, 'article includes required SEO/GEO sections', REQUIRED_SECTION_HEADINGS.every((heading) => headings.includes(heading)));
  requireCheck(label, 'article has exactly one decision table section', decisionTables.length === 1);
  requireCheck(label, 'decision table has at least 4 rows', decisionTables[0]?.table?.rows?.length >= 4);
  requireCheck(label, 'direct answer starts the article body', directAnswer?.body?.[0]?.length >= 180);
  ...
  requireCheck(label, 'internal links do not point to self', post.internalLinks.every((link) => link.href !== `/blogs/${post.slug}`));
  requireCheck(label, 'related slugs exist', post.relatedPostSlugs.every((slug) => slugs.has(slug)));
  requireCheck(label, 'visible content has no stale or spammy labels', BANNED_VISIBLE_PATTERNS.every((pattern) => !pattern.test(visibleText)));
  requireCheck(label, 'hero image uses WebP v8 path', post.heroImage.url.endsWith('hero-photo-v8.webp'));
  requireCheck(label, 'hero image has dimensions for layout stability', post.heroImage.width === 1600 && post.heroImage.height === 900);
  requireCheck(label, 'hero image has descriptive alt text', post.heroImage.alt.length >= 40 && post.heroImage.alt.includes(post.title));
  ...
  schemas.forEach((schema) => JSON.parse(JSON.stringify(schema)));
  const blogSchema = schemas[0];
  const breadcrumbSchema = schemas[1];
  const faqSchema = schemas[2];
  requireCheck(label, 'BlogPosting JSON-LD is complete', (
  ...
  requireCheck(label, 'Breadcrumb JSON-LD is complete', breadcrumbSchema['@type'] === 'BreadcrumbList' && breadcrumbSchema.itemListElement?.length === 3);
  requireCheck(label, 'FAQ JSON-LD is complete', faqSchema['@type'] === 'FAQPage' && faqSchema.mainEntity?.length === post.faq.length);
}

const failedChecks = checks.filter((item) => !item.passed);
const report = {
  valid: errors.length === 0 && failedChecks.length === 0,
  checklistScore: failedChecks.length === 0 ? 100 : Math.round(((checks.length - failedChecks.length) / checks.length) * 100),
  counts: {
    posts: posts.length,
    checks: checks.length,
    failedChecks: failedChecks.length,
    sitemapBlogUrls: entries.filter((entry) => entry.url?.startsWith(`${SITE_URL}/blogs/`)).length,
    sitemapImageUrls: entries.filter((entry) => entry.images?.some((image) => image.includes('hero-photo-v8.webp'))).length,
