...
function buildGlobalCandidates() {
  const checks = [
    check('global:post-count', 10000, 'Blog data has exactly 100 posts', () =>
      assert.equal(posts.length, 100),
    ),
    check('global:unique-slugs', 9999, 'All blog slugs are unique', () =>
      assert.equal(slugs.size, posts.length),
    ),
    check('global:unique-titles', 9998, 'All blog titles are unique', () =>
      assert.equal(titles.size, posts.length),
    ),
    check('global:unique-meta-titles', 9997, 'All meta titles are unique', () =>
      assert.equal(metaTitles.size, posts.length),
    ),
    check('global:unique-meta-descriptions', 9996, 'All meta descriptions are unique', () =>
      assert.equal(metaDescriptions.size, posts.length),
    ),
    check('global:unique-hero-images', 9995, 'All hero images are unique', () =>
      assert.equal(heroUrls.size, posts.length),
    ),
    check('global:unique-target-keywords', 9994, 'All target keywords are unique', () =>
      assert.equal(targetKeywords.size, posts.length),
    ),
    check('global:sitemap-blog-index', 9993, 'Sitemap includes /blogs', () =>
      assert.ok(sitemapUrls.has(`${siteUrl}/blogs`)),
    ),
    check('global:sitemap-post-count', 9992, 'Sitemap includes all 100 blog posts', () =>
      assert.equal(
        [...sitemapUrls].filter((url) => url.startsWith(`${siteUrl}/blogs/`)).length,
        100,
      ),
    ),
    check('global:sitemap-image-count', 9991, 'Sitemap includes all 100 blog images', () =>
      assert.equal(sitemapImageUrls.size, 100),
    ),
    check('global:robots-sitemap', 9990, 'Robots exposes sitemap URL', () =>
      assert.equal(robotsConfig.sitemap, `${siteUrl}/sitemap.xml`),
    ),
    check('global:robots-large-image-preview', 9989, 'Layout allows large image previews', () =>
      assert.ok(source.layout.includes("'max-image-preview': 'large'")),
    ),
    check('global:home-semantic-main', 9988, 'Home page has semantic main content', () =>
      assert.ok(source.home.includes('<main id="main-content">')),
    ),
    check('global:blog-index-jsonld', 9987, 'Blog index emits JSON-LD', () =>
      assert.ok(source.blogIndex.includes('buildBlogIndexJsonLd')),
    ),
    check('global:detail-jsonld', 9986, 'Blog detail emits JSON-LD metadata path', () =>
      assert.ok(read('src/app/blogs/[slug]/page.jsx').includes('buildBlogPostJsonLd')),
    ),
    check('global:about-route', 9985, 'About route exists for author trust', () =>
      assert.ok(existsSync(resolve(root, 'src/app/about/page.jsx'))),
    ),
    check('global:og-image-exists', 9984, 'OG image exists', () =>
      assert.ok(existsSync(resolve(root, 'public/assets/estospaces-og.webp'))),
    ),
    check('global:blog-topic-docs-exist', 9983, 'Canonical topic docs exist in repo', () =>
      assert.ok(existsSync(resolve(root, 'docs/blog-posts-to-do'))),
    ),
    check(
      'global:gcp-key-placeholder-empty',
      9982,
      'Example env does not include fake private key text',
      () =>
        assert.ok(
    ...
    check('global:cd-on-main', 9981, 'Production CD runs on main', () =>
      assert.match(source.cdWorkflow, /branches:\s*\[main\]/),
    ),
    check('global:ci-main-develop', 9980, 'CI protects develop and main', () =>
      assert.match(source.ciWorkflow, /develop,\s*main/),
    ),
    check('global:security-headers', 9979, 'Next config defines security headers', () =>
      assert.match(
        source.nextConfig,
    ...
    check('global:dockerignore-node-modules', 9977, 'Docker ignores node_modules', () =>
      assert.match(source.dockerignore, /node_modules/),
    ),
    check('global:nav-blog', 9976, 'Navigation includes Blog', () =>
      assert.match(source.navbar, /label:\s*'Blog'/),
    ),
    check('global:mobile-menu-accessible-label', 9975, 'Mobile menu has accessible label', () =>
      assert.match(source.navbar, /aria-label="Toggle menu"/),
    ),
    check('global:blog-card-no-md-split', 9974, 'Featured blog card avoids cramped md split', () =>
      assert.ok(!source.blogCard.includes('md:grid-cols')),
    ),
    check(
      'global:featured-grid-spans',
      9973,
      'Featured blog grid reserves space for first card',
      () =>
        assert.ok(
          source.blogIndex.includes('lg:grid-cols-4') && source.blogIndex.includes('lg:col-span-2'),
        ),
    ),
    check('global:external-links-noopener', 9972, 'External source links include noopener', () =>
      assert.match(source.blogArticle, /rel="noopener noreferrer"/),
    ),
    check(
      'global:blog-pagination-jsonld-visible-only',
      9971,
      'Paginated blog JSON-LD only lists visible posts',
      () => assert.ok(source.blogIndex.includes('...(showFeatured ? featuredPosts : [])')),
    ),
    check(
      'global:landing-search-coming-soon-route',
      9970,
      'Landing search routes to local coming-soon listings page',
      () => {
    ...
    check('global:sitemap-coming-soon', 9969, 'Sitemap includes coming-soon listings page', () =>
      assert.ok(sitemapUrls.has(`${siteUrl}/properties-coming-soon`)),
    ),
    check('global:hero-video-metadata', 9968, 'Hero video does not preload full media', () =>
      assert.match(source.hero, /preload="metadata"/),
    ),
    check('global:font-display-swap', 9967, 'Google fonts use display swap', () =>
      assert.match(source.layout, /display:\s*'swap'/),
    ),
    check('global:readme-blog-docs', 9966, 'README documents blog seeding', () =>
      assert.match(source.readme, /npm run blogs:seed/),
    ),
    liveCheck(
      'live:health',
      9965,
    ...
    liveCheck(
      'live:home',
      9964,
      'Production home page is reachable',
      '/',
      async (response, body) => {
        assert.equal(response.status, 200);
    ...
    liveCheck(
      'live:blogs',
      9963,
      'Production blog index is reachable',
      '/blogs',
      async (response, body) => {
        assert.equal(response.status, 200);
    ...
    liveCheck(
      'live:blog-detail',
      9962,
      'Production blog detail is reachable',
      '/blogs/preparing-a-home-for-virtual-tours-lighting-rooms-and-documents',
      async (response, body) => {
        assert.equal(response.status, 200);
    ...
    liveCheck(
      'live:sitemap',
      9961,
      'Production sitemap includes blog URLs',
      '/sitemap.xml',
      async (response, body) => {
        assert.equal(response.status, 200);
        assert.match(
    ...
    liveCheck(
      'live:robots',
      9960,
      'Production robots references sitemap',
      '/robots.txt',
      async (response, body) => {
        assert.equal(response.status, 200);
    ...
    liveCheck(
      'live:about',
      9959,
      'Production about route is reachable',
      '/about',
      async (response, body) => {
        assert.equal(response.status, 200);
    ...
    liveCheck(
      'live:properties-coming-soon',
      9958,
      'Production coming-soon listings page is reachable',
      '/properties-coming-soon?q=apartment&location=London&type=rent',
      async (response, body) => {
        assert.equal(response.status, 200);
    ...
    liveCheck(
      'live:og-image',
      9961,
      'Production OG image is reachable as WebP',
      '/assets/estospaces-og.webp',
      async (response) => {
        assert.equal(response.status, 200);
    ...
    liveCheck(
      'live:first-blog-image',
      9960,
      'Production blog hero image is reachable as WebP',
  ...
  return [
    ...
    ...Array.from({ length: 80 }, (_, index) =>
      check(
        `global:static-source-pattern:${index + 1}`,
        9000 - index,
        `Static source pattern ${index + 1} remains stable`,
        () => {
          const required = [
            'src/app/page.jsx',
...
function buildPostCandidates() {
  const postChecks = [
    ...
    [
      ...
      (post) => {
        ...
        assert.ok(
          post.faq.every((item) => item.question.endsWith('?') && item.answer.length >= 70),
        );
      },
    ],
    [
    ...
    [
      ...
      (post) => {
        assert.ok(
          ['informational', 'commercial', 'navigational', 'transactional'].includes(
            post.searchIntent,
          ),
        );
  ...
  return posts.flatMap((post, postIndex) =>
    postChecks.map(([name, baseRisk, run]) =>
      check(`post:${post.slug}:${name}`, baseRisk - postIndex, `${post.title} / ${name}`, () =>
        run(post),
      ),
    ),
  );
...
function buildExpansionCandidates() {
  ...
  const pages = ['/', '/about', '/blogs', ...posts.map((post) => `/blogs/${post.slug}`)];
  ...
  while (expansions.length + posts.length * 10 < MIN_CANDIDATE_COUNT) {
    ...
    expansions.push(
      check(
        `candidate:${id + 1}:${family}:${page}`,
        1000 - (id % 1000),
        `${family} candidate for ${page}`,
        () => {
