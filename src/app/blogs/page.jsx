...
export const metadata = {
  ...
  description:
    'Source-backed UK property guides for buyers, renters, landlords, investors and estate agents.',
  alternates: {
    canonical: '/blogs',
  },
  openGraph: {
    ...
    description:
      'Helpful UK property guides with practical checklists, FAQs, source notes and clear decision workflows.',
    ...
    <BlogChrome>
      ...
      <main className="pt-28" id="main-content">
        <section className="border-b border-gray-100 bg-white dark:border-gray-800 dark:bg-gray-950">
          <div className="container mx-auto px-4 py-10 sm:py-14">
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-gray-600 dark:text-gray-300">
              Helpful, source-backed articles for buyers, renters, landlords, investors and estate
              agents. Every guide starts with the direct answer, then adds checklists, definitions,
              FAQs and next steps.
            </p>
          </div>
        </section>
        ...
        <section className="container mx-auto px-4 pb-14">
          <aside className="mb-7 border-l-4 border-primary bg-orange-50 p-5 text-sm leading-6 text-gray-700">
            Blog content is general information, not legal, financial, tax, surveying, or safety
            advice. Product-related articles may discuss workflows that are limited or still being
            evaluated during private beta.
          </aside>
          ...
          <div className="mb-6 border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-950">
            <form action="/blogs" className="grid gap-3 md:grid-cols-[1fr_220px_220px_auto]">
              ...
              >
                ...
                {result.tags.slice(0, 80).map((item) => (
                  <option key={item} value={item}>
                    {item}
                  </option>
                ))}
              ...
              <button
                className="min-h-12 bg-primary px-6 font-bold text-white hover:bg-opacity-90"
                type="submit"
              >
                Search
              </button>
...
function Pagination({ result, query, category, tag }) {
  ...
  return (
    <nav className="mt-10 flex items-center justify-between" aria-label="Blog pagination">
      {previous ? (
        <a
          className="border border-gray-200 bg-white px-4 py-2 text-sm font-semibold hover:border-primary dark:border-gray-700 dark:bg-gray-950"
          href={pageHref(previous, query, category, tag)}
        >
          Previous
        </a>
      ) : (
      ...
      {next ? (
        <a
          className="border border-gray-200 bg-white px-4 py-2 text-sm font-semibold hover:border-primary dark:border-gray-700 dark:bg-gray-950"
          href={pageHref(next, query, category, tag)}
        >
          Next
        </a>
      ) : (
