...
export default function BlogCard({ post, featured = false, className = '' }) {
  return (
    <article
      className={`group min-w-0 overflow-hidden bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-sm ${featured ? 'lg:grid lg:grid-cols-[1fr_1fr]' : ''} ${className}`}
    >
      ...
      <div className={`min-w-0 ${featured ? 'p-6 sm:p-7 lg:p-8' : 'p-5 sm:p-6'}`}>
        ...
        <h2
          className={`${featured ? 'text-2xl sm:text-3xl' : 'text-xl'} mt-3 text-balance font-bold leading-tight text-gray-950 dark:text-white`}
        >
