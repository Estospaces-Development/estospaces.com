    ...
    >
      <header className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950 pt-28">
        <div className="container mx-auto max-w-5xl px-4 py-10 sm:py-14">
          ...
          <a
            href={categoryHref}
            className="text-sm font-semibold uppercase tracking-normal text-primary hover:underline"
          >
            {post.category}
          </a>
          <h1
            itemProp="headline"
            className="mt-3 max-w-4xl text-4xl font-bold leading-tight text-gray-950 dark:text-white sm:text-5xl"
          >
            {post.title}
          </h1>
          <p
            itemProp="description"
            className="mt-5 max-w-3xl text-lg leading-8 text-gray-600 dark:text-gray-300"
          >
          ...
          <div className="mt-6 flex flex-wrap gap-3 text-sm text-gray-500 dark:text-gray-400">
            <span>
              ...
              <a
                ...
                className="font-semibold text-gray-700 hover:text-primary dark:text-gray-200"
              >
                {post.author.name}
              </a>
            </span>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            {post.tags.slice(0, 6).map((tag) => (
              <a
                ...
                className="border border-gray-200 bg-white px-3 py-1 text-xs font-semibold text-gray-600 hover:border-primary hover:text-primary dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300"
              >
                {tag}
              </a>
            ))}
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[260px_1fr]">
          <aside className="lg:sticky lg:top-28 lg:self-start">
            <div className="border border-gray-100 bg-gray-50 p-5 dark:border-gray-800 dark:bg-gray-950">
              <h2 className="text-sm font-bold uppercase tracking-normal text-gray-950 dark:text-white">
                Table of contents
              </h2>
              ...
              <ul className="mt-4 space-y-3 text-sm">
                {post.internalLinks.slice(0, 5).map((link) => (
                  <li key={link.href}>
                    <a
                      href={link.href}
                      className="font-semibold text-gray-800 hover:text-primary dark:text-gray-200"
                    >
                      {link.title}
                    </a>
          ...
          <div className="prose prose-gray max-w-none dark:prose-invert">
            {post.content.sections.map((section) => (
              <section
                ...
                className="scroll-mt-28 border-b border-gray-100 py-8 last:border-b-0 dark:border-gray-800"
              >
                      ...
                      <thead>
                        <tr>
                          {section.table.headers.map((heading) => (
                            <th
                              key={heading}
                              className="border border-gray-200 bg-gray-50 px-4 py-3 font-semibold dark:border-gray-700 dark:bg-gray-800"
                            >
                              {heading}
                            </th>
                          ))}
                        ...
                        {section.table.rows.map((row) => (
                          <tr key={row.join('|')}>
                            {row.map((cell) => (
                              <td
                                key={cell}
                                className="border border-gray-200 px-4 py-3 dark:border-gray-700"
                              >
                                {cell}
                              </td>
                            ))}
              ...
              <div className="mt-5 grid gap-4 md:grid-cols-2">
                {post.internalLinks.map((link) => (
                  <a
                    key={link.href}
                    href={link.href}
                    className="border border-gray-100 bg-gray-50 p-4 hover:border-primary dark:border-gray-800 dark:bg-gray-950"
                  >
            ...
            <section id="frequently-asked-questions" className="py-8">
              ...
              <div className="mt-5 space-y-4">
                {post.faq.map((item) => (
                  <details
                    key={item.question}
                    className="border border-gray-100 bg-gray-50 p-4 dark:border-gray-800 dark:bg-gray-950"
                    open
                  >
            ...
            <div className="mt-6 grid gap-5 md:grid-cols-3">
              {relatedPosts.map((related) => (
                <a
                  ...
                  className="border border-gray-100 bg-white p-5 shadow-sm hover:border-primary dark:border-gray-800 dark:bg-gray-950"
                >
