...
const request = (path, { method = 'POST', body, headers = {} } = {}) =>
  new Request(`https://estospaces.com${path}`, {
    method,
    headers: {
...
test('allowed CORS preflight is handled for landing API routes', async () => {
  const response = reservationRoute.OPTIONS(
    request('/api/send-reservation-email', {
      method: 'OPTIONS',
      headers: {
...
test('reservation API validates email and accepts valid local request without external provider', async () => {
  const malformed = await reservationRoute.POST(
    new Request('https://estospaces.com/api/send-reservation-email', {
      method: 'POST',
  ...
  const invalid = await reservationRoute.POST(
    request('/api/send-reservation-email', {
      body: {
        userType: 'buyer',
        name: 'Maya User',
  ...
  const valid = await reservationRoute.POST(
    request('/api/send-reservation-email', {
      body: {
        market: 'england',
        userType: 'buyer',
...
test('reservation API rejects unsupported launch markets', async () => {
  const response = await reservationRoute.POST(
    request('/api/send-reservation-email', {
      body: {
        market: 'usa',
        userType: 'buyer',
...
test('reservation API formats market phone numbers before sheet append', async () => {
  ...
  try {
    const india = await reservationRoute.POST(
      request('/api/send-reservation-email', {
        body: {
          market: 'india',
          userType: 'buyer',
    ...
    const england = await reservationRoute.POST(
      request('/api/send-reservation-email', {
        body: {
          market: 'england',
          userType: 'renter',
...
test('reservation API appends Reserve Your Spot leads to configured Google Sheet', async () => {
  ...
  try {
    ...
    assert.match(
      calls[0].url,
      /spreadsheets\/1lcHZXqllQ6JT-pnnA6fN7cVqRt8xhZtU7TbBnsroUOA\/values\/A1%3AQ1/,
    );
    assert.match(
      calls[1].url,
      /spreadsheets\/1lcHZXqllQ6JT-pnnA6fN7cVqRt8xhZtU7TbBnsroUOA\/values\/A%3AQ:append/,
    );
...
test('landing search supports implemented app markets without publishing a rollout claim', () => {
  const searchUrlSource = readFileSync(resolve(process.cwd(), 'src/lib/search-url.js'), 'utf8');
  ...
  assert.match(searchUrlSource, /allowedMarkets/);
  assert.doesNotMatch(layoutSource, /India and England/);
  assert.doesNotMatch(pageSource, /areaServed/);
});
...
test('reservation API rejects duplicate reservations by email or phone', async () => {
  ...
  const first = await reservationRoute.POST(
    request('/api/send-reservation-email', { body: firstBody }),
  );
  assert.equal(first.status, 200);

  const duplicateEmail = await reservationRoute.POST(
    request('/api/send-reservation-email', { body: firstBody }),
  );
  assert.equal(duplicateEmail.status, 409);
  assert.match((await duplicateEmail.json()).error, /already reserved/i);

  const duplicatePhone = await reservationRoute.POST(
    request('/api/send-reservation-email', {
      body: {
        ...firstBody,
...
test('newsletter and chat APIs keep the same no-provider behavior', async () => {
  const newsletter = await newsletterRoute.POST(
    request('/api/send-newsletter-notification', {
      body: {
  ...
  const chatStart = await chatStartRoute.POST(
    request('/api/live-chat/start', {
      body: {
  ...
  const chatMessage = await chatMessageRoute.POST(
    request('/api/live-chat/message', {
      body: {
        visitorId: 'visitor-123',
...
test('newsletter and chat email APIs throttle repeated public submissions', async () => {
  ...
  const limitedNewsletter = await newsletterRoute.POST(
    request('/api/send-newsletter-notification', {
      body: newsletterBody,
  ...
  for (let i = 0; i < 5; i += 1) {
    const response = await chatMessageRoute.POST(
      request('/api/live-chat/message', {
        body: chatBody,
  ...
  const limitedChat = await chatMessageRoute.POST(
    request('/api/live-chat/message', {
      body: chatBody,
